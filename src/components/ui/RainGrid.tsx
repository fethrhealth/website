 ...
 * On mount each line animates 0 → baseY with a random staggered delay + easeOut.
 *
 * Props:
 *   variant     — 'dark' (black bg, default) | 'light' (white bg, subtle gray lines)
...
const EDGE_Y_FRAC   = 0.22   // center lines end at 22 % of canvas height
const CENTER_Y_FRAC = 0.78   // edge lines end at 78 % of canvas height

// Color schemes
const COLORS = {
  dark:  { bg: '#101113', r: 111, g: 121, b: 136, a: 0.4 },
  light: { bg: '#ffffff', r: 0, g: 0, b: 0, a: 0.05 },
}
...
function buildLines(W: number, H: number, immediate: boolean): Line[] {
  ...
  return Array.from({ length: LINE_COUNT }, (_, i) => {
    ...
    const t     = ((x - cx) / hw) ** 2             // 0 at center, 1 at edges
    const baseY = edgeY + (centerY - edgeY) * t    // center → short, edges → deep
...
export function RainGrid({ variant = 'dark', fillHeight = false, className }: RainGridProps) {
  ...
  useEffect(() => {
    ...
    const init = (immediate: boolean) => {
      ...
      if (!immediate) mountTimeRef.current = performance.now()
    }
    ...
    const draw = (now: number) => {
      ...
      for (const line of linesRef.current) {
        ...
        if (line.entered) {
          ...
          line.currentY = lerp(line.currentY, line.targetY, LERP_SPEED)
        }
