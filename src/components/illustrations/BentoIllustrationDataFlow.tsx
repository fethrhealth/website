...
const WorkspaceColourIcon = () => (
  <Image src="/assets/logos/data-logos/epic-logo.webp" alt="Workspace" width={18} height={18} />
)

const CompanyColourIcon = () => (
  <Image src="/assets/logos/data-logos/teams-logo.svg" alt="Company" width={18} height={18} />
)

const DealColourIcon = () => (
  <Image src="/assets/logos/data-logos/snowflake-logo.svg" alt="Deal" width={18} height={18} />
)
...
function DataCard({
  icon, label, count, gridClass, recordText, tag
}: {
  icon: ReactNode
  label: string
  count: number
  ...
  return (
    ...
    >
      ...
      <p className="flex items-baseline font-medium leading-3 tracking-[-0.02em]">
        ...
        <span className="text-[10px] text-black-700 pl-1">{recordText}</span>
      </p>
    </div>
  )
}
...
export function BentoIllustrationDataFlow(): ReactNode {
  ...
  const [cardCounts, setCardCounts] = useState({ workspace: 5283493, company: 3128, deal: 2383495 })
      ...
      <div className="relative grid scale-[70%] grid-rows-[calc(50%+12px)_calc(50%-15px)] items-center justify-items-center md:scale-100 xl:scale-90 2xl:scale-100">
        ...
        >
          ...
          <div className="absolute inset-px flex items-center justify-center rounded-[11px] bg-[#FFFFFF]">
            <Image src="/assets/logos/icon.svg" alt="" width={36} height={32} />
          </div>
        </div>
