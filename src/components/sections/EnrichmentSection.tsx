 ...
 * EnrichmentSection
 *
 * Three-column image grid with dot-grid card backgrounds, diagonal hatching,
 * dashed rules, and vertical guide lines above. Reusable across platform pages.
 *
 * Images should be square or close to square (aspect-square card area).
 */
...
export interface EnrichmentItem {
  /** Unique id — used as SVG pattern id, must be unique across the page */
  id: string
  image: string
  imageWidth: number
  imageHeight: number
...
export function EnrichmentSection({ heading, subheading, items }: EnrichmentSectionProps) {
  return (
              ...
              {items.map((item) => (
                <div key={item.id} className="grid grid-cols-12 bg-primary-background">
                  <div className="col-[2/-2] flex flex-col py-7">
                    ...
                    {/* Image with dot-grid background */}
